...
2. **Review phase** (`Stop`, `asyncRewake`): the `--review` mode reads the per-session records, builds review packets (each `new_string` + context lines from the on-disk file), applies a comment-marker pre-filter to skip code-only edits, then batches the surviving hunks into `claude -p` calls (Haiku; up to 20 hunks per call, batches run in parallel up to `--max-parallel`). The good-comment rules live in the reviewer's system prompt — a stable, cacheable prefix — while only the per-hunk code packets vary on stdin, so repeated calls reuse the prompt cache. Each call runs lean — no settings loaded (hence no hooks or plugin-sync, so the reviewer never re-runs this plugin's own `SessionStart`), no MCP servers, tools off, and `--effort low` (thinking disabled via `MAX_THINKING_TOKENS=0` on the child). Auth still resolves from the keychain, so OAuth keeps working without `--bare`'s API-key requirement. A single-shot classification needs no deliberation, so each call lands in ~2s. Raise `--effort` to re-enable proportional thinking.
